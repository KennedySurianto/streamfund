    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
        <div className="px-8 py-6 border-b border-gray-100 bg-gray-50">
          ...
          <p className="text-sm text-gray-500">Manage your account details and stream settings.</p>
        </div>

        <form onSubmit={handleUpdateProfile} className="p-8 space-y-8">
          {updateError && <div className="p-3 bg-red-50 text-red-600 rounded-md text-sm">{updateError}</div>}
          {updateSuccess && <div className="p-3 bg-green-50 text-green-600 rounded-md text-sm">{updateSuccess}</div>}

          {/* Avatar Upload */}
          <div className="flex items-center space-x-6">
            <div className="relative w-24 h-24 rounded-full overflow-hidden bg-gray-100 border-4 border-white shadow-lg">
              {formData.profile_picture ? (
                <Image src={formData.profile_picture} alt="Avatar" fill className="object-cover" />
              ) : (
            ...
            <div>
              <label className="cursor-pointer bg-white px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 flex items-center transition-colors">
                ...
                Change Picture
                <input type="file" className="hidden" accept="image/png, image/jpeg, image/gif" onChange={handleImageChange} />
              </label>
          ...
          <div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  type="password" value={formData.old_password}
